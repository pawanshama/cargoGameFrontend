...
const Header: React.FC<HeaderProps> = ({ pageHeading }) => {
  ...
  const handleNotificationClick = async () => {
    ...
    try {
      await axios.get(
        "https://corgi-in-space-backend-production.up.railway.app/api/notifications/all",
    ...
    const fetchUnread = async () => {
      try {
        const { data } = await axios.get(
          "https://corgi-in-space-backend-production.up.railway.app/api/notifications/unread-count",
          { headers: { Authorization: `tma ${initData}` } },
        );
